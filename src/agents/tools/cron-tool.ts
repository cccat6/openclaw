...
const CronJobInputSchema = Type.Object(
  {
    ...
    fallbackSessionKey: Type.Optional(Type.String()),
  },
  { additionalProperties: false },
);
...
function sanitizeCronAddJobForGateway(raw: unknown): Record<string, unknown> | null {
  ...
  if (typeof input.fallbackSessionKey === "string") {
    const trimmed = input.fallbackSessionKey.trim();
    if (trimmed) {
...
function sanitizeCronPatchForGateway(raw: unknown): Record<string, unknown> | null {
  ...
  if ("fallbackSessionKey" in input) {
    if (input.fallbackSessionKey === null) {
      next.fallbackSessionKey = null;
    } else if (typeof input.fallbackSessionKey === "string") {
      const trimmed = input.fallbackSessionKey.trim();
      if (trimmed) {
        next.fallbackSessionKey = trimmed;
      }
...
TOOL CALL ENVELOPE (this tool):
{
  ...
  "fallbackSessionKey": "session key" // optional helper for add/update patch routing
}
...
ADD JOB SCHEMA (maps to cron.add):
{
  ...
  "fallbackSessionKey": "session key" // optional; where to post fallback notice when outbound delivery fails
}
...
- when outbound announce delivery fails, tool/runtime can fall back to fallbackSessionKey (defaults to current tool session when available)
      ...
      switch (action) {
        ...
        case "add": {
          ...
          if (
            job &&
            typeof job === "object" &&
            !("fallbackSessionKey" in job) &&
            typeof opts?.agentSessionKey === "string" &&
            opts.agentSessionKey.trim()
          ) {
            (job as { fallbackSessionKey?: string }).fallbackSessionKey =
              opts.agentSessionKey.trim();
          }
